...
### REQUIRES_NEW
**항상 새로운 트랜잭션을 생성**하고, **기존 트랜잭션은 일시 중단**합니다.
...
### SUPPORTS
**기존 트랜잭션이 있으면 참여**하고, 없으면 **트랜잭션 없이 실행**합니다.
...
### NOT_SUPPORTED
**트랜잭션 없이 실행**하고, **기존 트랜잭션이 있으면 일시 중단**합니다.
...
기존 트랜잭션이 `일시 중단(suspend)`되면 해당 [커넥션은 반환되지 않고 점유된 채](#requires_new-주의사항)로 
존재하므로 주의해야 합니다.

### MANDATORY
**반드시 기존 트랜잭션이 필수**이고, **없으면 예외가 발생**합니다.
**항상 트랜잭션 내에서만 호출되어야 하는 메서드**에 사용하면
...
### NEVER
`MANDATORY`와 정반대 개념으로 **트랜잭션이 있으면 예외가 발생**하고, **트랜잭션 없이만 실행 가능**합니다.
