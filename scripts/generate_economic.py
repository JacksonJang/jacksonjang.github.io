...
def _escape_multiline_strings(raw_json: str) -> str:
    """Convert unescaped newlines/tab characters inside JSON strings."""
    ...
    for ch in raw_json:
        if in_string:
            ...
            result_chars.append(ch)
            if ch == '"':
                # Count trailing backslashes to see if this quote starts a string
                backslash_count = 0
                idx = len(result_chars) - 2
                while idx >= 0 and result_chars[idx] == "\\":
                    backslash_count += 1
                    idx -= 1
...
def sanitize_json_text(raw_json: str) -> str:
    cleaned = raw_json.replace("\r\n", "\n")
    cleaned = cleaned.replace("\u0000", "")
    return _escape_multiline_strings(cleaned)
...
"""
    ...
    try:
        ...
        # web_search/tooling은 SDK에 따라 지원 여부가 달라서 우선 시도 후 실패 시 제거합니다.
        create_kwargs["tools"] = [{"type": "web_search"}]
        create_kwargs["response_format"] = {"type": "json_object"}

        def _call_with_backoff(kwargs: Dict[str, Any]) -> Any:
            try:
                ...
                if "unexpected keyword argument" in msg:
                    ...
                    if "response_format" in kwargs and "response_format" in msg:
                        ...
                        print("[WARN] Responses API가 response_format을 지원하지 않아 제거합니다.")
                        trimmed = True
                    if "tools" in kwargs and "tools" in msg:
                        kwargs = dict(kwargs)
                        kwargs.pop("tools", None)
                        print("[WARN] Responses API가 tools 매개변수를 지원하지 않아 제거합니다.")
                        trimmed = True
                    if trimmed:
                        return _call_with_backoff(kwargs)
                raise

        resp = _call_with_backoff(create_kwargs)
        ...
        try:
            ...
            parse_error = decode_err
            fixed_txt = sanitize_json_text(txt)
            if fixed_txt != txt:
                try:
                    data = _json.loads(fixed_txt)
                    print("[WARN] JSON 내 개행/제어문자 보정 후 재시도 성공")
                    txt = fixed_txt
                except _json.JSONDecodeError as decode_err2:
                    parse_error = decode_err2

        if data is None:
            snippet = txt[:500]
            if parse_error:
                print("[WARN] AI JSON 파싱 실패:", parse_error)
            else:
                print("[WARN] AI JSON 파싱 실패: Unknown error")
